...
function ItemDAO(database) {
    ...
    this.getNumItems = function(category, callback) {
        ...
        if (category === "All"){
            ...
            this.db.collection('item',function (err,collection) {
                if(err) throw err;
                collection.find({"category":category}).count(function(err,result){
                    assert.equal(err,null);
                    callback(result);
                });
            });
    ...
    this.addReview = function(itemId, comment, name, stars, callback) {
        ...
        var query = this.db.collection("item").update({"_id":itemId},
            {$push:{reviews:reviewDoc}});
        callback(query);


        // db.item.update({"_id":'1'},{$push:{reviews:{
        //     name: 'name',
        //     comment: 'comment',
        //     stars: 4,
        //     date: Date.now()
        // }}});
        // //db.item.update({$push:{reviews:reviewDoc}})
